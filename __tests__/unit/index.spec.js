...
const fakeCollection = {
  ...
  insertMany: jest.fn(arr =>
    Promise.resolve({ insertedCount: arr.length, ops: arr })
  ),
  ...
  it('call connect with uri & opts', () => {
    ...
    return adapter
      ...
      .then(() => {
        ...
        expect(MongoClient.connect).toHaveBeenCalledWith(
          adapter.uri,
          Object.assign(adapter.opts, { promiseLibrary: jasmine.any(Function) })
        );
  ...
  it('call updateMany', () => {
    ...
    return adapter
      ...
      .then(res => {
        ...
        expect(adapter.collection.updateMany).toHaveBeenCalledWith(
          query,
          update
        );
      });
